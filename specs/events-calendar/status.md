...
- **state**: `in-progress`
- **fase**: ejecución de tasks. T1–T10 done; UI funcionalmente completa. Pendiente bloque E2E (T11–T15) y cierre (T16).
...
UI **funcionalmente completa**. Bloque siguiente: **E2E + a11y**
(T11–T15) que puede ejecutarse en paralelo:

- **T11** — E2E creación 4 tipos (R3.1–R3.4).
- **T12** — E2E editar/borrar/toggle/validaciones (R3.5, R3.6, R4.1–R4.3).
- **T13** — E2E navegación + panel + multi-día (R2.1–R2.4).
- **T14** — E2E bloqueo R1.4 (override de `localStorage` en Playwright).
- **T15** — E2E accessibility (teclado puro + axe-core en cada flow).

Antes de arrancar T11 conviene una **pasada visual humana** del
`/calendar` (crear evento, editar, borrar, toggle, navegación), porque
el smoke del dev server sólo verifica el path skeleton SSR.
