...
7. Los commits posteriores citan `AMD-NNN` además de `R*.*`. Ejemplo:
   `feat({{SERVICE_NAME}}): T11 - consent gate [R3.2 AMD-001] AB#12399`

Un Amendment NO es un bug Tipo B. Tipo B son cosas que estaban mal desde
el inicio; un Amendment es un evento nuevo posterior a la aprobación.
Mantener la distinción mejora la métrica de calidad de spec authoring
(si todo se mete como Tipo B, el agente nunca aprende).
