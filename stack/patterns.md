...
```
<type>(demo1-ai-dlc): T<n> - <descripción corta> [R<x>.<y>] AB#<workitem-id>
```
...
```
feat(demo1-ai-dlc): T1 - login con JWT en cookie httpOnly [R1.3] AB#42
fix(demo1-ai-dlc): T3 - rechazar token con iss inválido [R1.5] AB#48
refactor(demo1-ai-dlc): T7 - extraer jwtVerifier a infra [R1.3] AB#42
chore(demo1-ai-dlc): bump next a 15.0.3
docs(demo1-ai-dlc): ADR 0001 sobre Clean Architecture
```
...
- **Campos obligatorios en cada log entry**:
  - `requestId` (UUID v4 generado en middleware, propagado por
    AsyncLocalStorage).
  - `service: "demo1-ai-dlc"` (default del child logger).
